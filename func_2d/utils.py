...
def get_network(args, net, use_gpu=True, gpu_device=0, distribution=True):
    ...
    MODIFIED: Now accepts args as parameter instead of using global cfg
    """
    ...
    if net == 'sam2':
        ...
        # MODIFIED: Check if args has sam_config and sam_ckpt
        if not hasattr(args, 'sam_config') or not hasattr(args, 'sam_ckpt'):
            raise ValueError("args must have 'sam_config' and 'sam_ckpt' attributes")
    ...
    if use_gpu:
        if distribution != 'none':
            # MODIFIED: Handle distributed training
            if hasattr(args, 'distributed') and args.distributed != 'none':
...
def dice_coeff(input, target):
    ...
    # MODIFIED: Handle device more gracefully
    if input.is_cuda:
...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse=False, points=None, args=None):
    """
    MODIFIED: Accept args as parameter instead of using global
    """
    ...
    if c == 2:  # for REFUGE multi mask output
        pred_disc, pred_cup = pred_masks[:, 0, :, :].unsqueeze(1).expand(b, 3, h, w), \
                              pred_masks[:, 1, :, :].unsqueeze(1).expand(b, 3, h, w)
        gt_disc, gt_cup = gt_masks[:, 0, :, :].unsqueeze(1).expand(b, 3, h, w), \
...
def random_click(mask, point_label=1):
    ...
    if len(indices) == 0:
        h, w = mask.shape
        return point_label, np.array([h // 2, w // 2])
